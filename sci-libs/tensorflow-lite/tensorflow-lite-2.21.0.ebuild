...
# build ALL vendored dependencies (abseil-cpp, eigen, flatbuffers, farmhash,
# fft2d, gemmlowp, ruy, cpuinfo, XNNPACK, pthreadpool, FP16, NEON_2_SSE,
# ml_dtypes, etc.).  These are statically linked into libtensorflow-lite.a.
#
# NOTE: flatbuffers MUST be vendored — TF 2.21.0's schema_generated.h has a
# static_assert requiring flatbuffers minor version 9.  System flatbuffers
# (e.g. dev-libs/flatbuffers-25.x with minor=12) is incompatible.  The
# vendored flatbuffers headers are NOT installed to /usr/include to avoid
# collisions with the system package.
...
src_install() {
	...
	# dev-libs/flatbuffers.  The vendored version (minor=9) is incompatible
	# with the system version anyway.  The C API does not expose flatbuffers
	# types, so bgremover and other C API consumers are unaffected.
