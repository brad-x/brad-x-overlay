...
SRC_URI="https://projects.savoirfairelinux.com/attachments/download/14805/${P}.tar.gz"

LICENSE="GPL-3"
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="debug doxygen gnome kde gsm networkmanager opus pulseaudio speex static-libs"
...
CDEPEND="dev-cpp/commoncpp2
	...
	gnome? ( dev-libs/atk
		...
		x11-libs/libSM )
	gsm? ( media-sound/gsm )
	kde? ( kde-base/kdepimlibs kde-base/kdelibs )
...
src_prepare() {
	#epatch "${FILESDIR}"/sflphone-externalpjsip-1.4.1.patch
	#cd "${S}/daemon"||die
	...
	if use kde; then
		S="${S}/kde"
		sed -i -e "s|\.\.|\.\./sflphone-1.4.1/kde|" ../kde/src/klib/kcfg_settings.kcfgc||die
...
src_configure() {
	...
	cd "${WORKDIR}/${P}/daemon/libs/pjproject-2.2.1"
	econf
